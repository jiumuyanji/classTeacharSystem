...
import kotlinx.android.synthetic.main.activity_readvisit.*

class ReadvisitActivity : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_readvisit)
        ...
        delete.setOnClickListener {
            AlertDialog.Builder(this).apply {
                setTitle("确认删除")
                setMessage("会删除该学生的家访信息")
                setCancelable(false)
                setPositiveButton("ok"){_, _->
                    db.delete("visit","name = ? and time = ?", arrayOf(extraName.toString(),extraTime.toString()))
                }
